...
impl Cpu {
    ...
    pub fn decode_and_exec(&mut self) {
        //anding every 4 bits
        //to only get those bits in whichever position such that it matters
        ...
        match (nibble1, nibble2, nibble3, nibble4) {
            ...
            (3, _, _, _) => {
                if self.registers[nibble2 as usize]
                    == (((nibble3 as u16) << 4) | (nibble4 as u16)) as u8
                {
                    self.pc += 2;
                }
            }
            ...
            (7, 0..=15, _, _) => {
                ...
                self.registers[nibble2 as usize] =
                    self.registers[nibble2 as usize].wrapping_add(nn);
            }
            (8, _, _, 0) => {
            ...
            (8, _, _, 4) => {
                let sum = self.registers[nibble2 as usize] as u16
                    + self.registers[nibble3 as usize] as u16;
            ...
            (8, _, _, 5) => {
                match (
                    self.registers[nibble2 as usize],
                    self.registers[nibble3 as usize],
                ) {
                ...
                self.registers[nibble2 as usize] =
                    self.registers[nibble2 as usize].wrapping_sub(self.registers[nibble3 as usize]);
            }
            (8, _, _, 6) => {
            ...
            (8, _, _, 7) => {
                ...
                let sum =
                    self.registers[nibble3 as usize].wrapping_sub(self.registers[nibble2 as usize]);
            ...
            (0xF, _, 2, 9) => {
                self.idx_register = u16::from(self.registers[nibble2 as usize]) * 0x05_u16
            }
            (0xF, _, 3, 3) => {
